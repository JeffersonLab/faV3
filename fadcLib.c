...
faGStatus(int sflag)
{
  ...
  for(ifa=0; ifa<nfadc; ifa++)
    {
      ...
      printf("%s",
	     (st[id].sparse_control &FA_SPARSE_CONTROL_BYPASS) ?"Bypassed":" Enabled");
  ...
  for(ifa=0; ifa<nfadc; ifa++)
      ...
      for(ichan = 0; ichan < FA_MAX_ADC_CHANNELS; ichan++)
	{
	  ...
	  printf("%d", (st[id].adc_thres[ichan] & FA_THR_ACCUMULATOR_SCALER_MODE_MASK) ? 1 : 0);
...
faGetSparsificationMode(int id)
{
  ...
  FALOCK;
  mode = (int) (vmeRead32(&FAp[id]->sparse_control) & FA_SPARSE_CONTROL_BYPASS);
 ...
 */
int
faSetAccumulatorScalerMode(int id, unsigned short chmask)
{
  int ii;
  ...
  for(ii=0;ii<FA_MAX_ADC_CHANNELS;ii++)
      ...
      if(ii%2 == 0)
	{
	  ...
	  if((1<<ii)&chmask)     lovalue |= FA_THR_ACCUMULATOR_SCALER_MODE_MASK;
	  else                   lovalue &=~FA_THR_ACCUMULATOR_SCALER_MODE_MASK;

	  if((1<<(ii+1))&chmask) hivalue |= FA_THR_ACCUMULATOR_SCALER_MODE_MASK;
	  else                   hivalue &=~FA_THR_ACCUMULATOR_SCALER_MODE_MASK;
 ...
 */
int
faGSetAccumulatorScalerMode(unsigned short chmask)
{
  int ifa = 0, rval = OK;
