...
init(char *choice)
{
  ...
  faV3Init(vme_addr, 1<<19, ninit, iflag);
  if(nfaV3 <= 0)
    {
...
setped(char *choice)
{
  ...
  if(faV3SampleConfig(FAV3_SLOT, nsamples, maxped) != OK)
    {
      printf("%s: ERROR from faV3SampleConfig\n", __func__);
      return ERROR;
    }
...
getped(char *choice)
{
  ...
  if(faV3ReadAllChannelSamples(FAV3_SLOT, channel_data) <= 0)
    {
      printf("%s: ERROR from faV3ReadAllChannelSamples\n",
	     __func__);
      return -1;
    }
