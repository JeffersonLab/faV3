/*************************************************************************
 *
 *  compton_list.c - Readout of faV3 with firmware designed for the
 *                   compton polarimeter photon detector.
 *
 *                   This one uses the TI as a clock,sync, and trigger
 *                   source to the faV3.
 *
 */
...
#include "faV3-Compton.h"     /* Compton Polarimeter - Photon Detector firmware */
...
char *default_config = "/home/compton/cfg/faV3-Compton.cfg";
...
rocDownload()
{
  ...
  vmeSetQuietFlag(1);
  faV3ComptonInit(FADC_ADDR, FADC_INCR, NFAV3, iflag);
  vmeSetQuietFlag(0);

  faV3ComptonSelectTrigger(faV3Slot(0), 1);  // 1: TI trigger
...
rocPrestart()
{
  ...
  int rval = 0;
  if(strlen(rol->usrConfig) > 0)
    rval = faV3Config(rol->usrConfig);
  else
    rval = faV3Config(default_config);
...
/*
  Local Variables:
  compile-command: "make -k compton_list.so"
  End:
 */
